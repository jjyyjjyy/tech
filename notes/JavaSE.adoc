...
线程A产生一个事件序列stem:[a_0,a_1,...].
由于线程中往往包含循环, 因此一条程序语句可以产生多次事件.
用stem:[a_i^j]表示事件stem:[a_i]的第stem:[j]次发生. 如果事件a在事件b之前发生, 则称a先于b, 记作stem:[a->b].

令stem:[a_0,a_1]表示事件, 且stem:[a_0->a_1], stem:[I_A]=interval(stem:[a_0->a_1])表示stem:[a_0和a_1]事件之间的间隔.
如果stem:[a_1->b_0],则stem:[I_A->I_B].多个不存在stem:[->]的关系称为 *并发的*.
...
.FilterLock.java
----
include::../basic-knowledge/src/main/java/me/jy/lang/thread/juc/lock/FilterLock.java[]
----

[source,java]
.BakeryLock.java
----
include::../basic-knowledge/src/main/java/me/jy/lang/thread/juc/lock/BakeryLock.java[]
----
