...
当集群用户需要在其 pod 中使用持久化存储(PV)时, 他们首先创建持久卷声明
(Persistent VolumeClaim, 简称 PVC) 清单, 指定所需要的最低容量要求和访问模式,
然后用户将PV提交给Kubernetes apiserver, Kubernetes将找到可匹
配的PV并将其绑定到PVC。
...
=== https://docs.helm.sh/developing_charts/#charts[常用命令]
