...
# 安装metrics-server监控
helm install metrics-server stable/metrics-server --namespace kubernetes-dashboard --set args={"--kubelet-insecure-tls=true,--kubelet-preferred-address-types=InternalIP\,Hostname\,ExternalIP"} --set image.repository=registry.cn-hangzhou.aliyuncs.com/google_containers/metrics-server-amd64
----
