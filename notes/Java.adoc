...
无锁: stem:[ubrace("hash")_(25位) ubrace("age")_(4位) ubrace("bias_lock")_(1位)^0 ubrace("lock")_(2位)^01]

偏向锁: stem:[ubrace("thread")_(23位) ubrace("epoch")_(2位) ubrace("age")_(4位) ubrace("bias_lock")_(1位)^1 ubrace("lock")_(2位)^01]

轻量级锁: stem:[ubrace("ptr_lock_record")_(30位) ubrace("lock")_(2位)^00]

重量级锁: stem:[ubrace("ptr_monitor")_(30位) ubrace("lock")_(2位)^10]
...
* 新生代 `Serial` 使用复制算法. 暂停所有用户线程.
* 老年代 `Serial Old` 使用标记-整理算法, 暂停所有线程.
...
[.lead]
Parallel垃圾收集器使用 *多线程* 回收内存.
垃圾回收时会暂停所有的应用线程.
使用 `-XX:+UseParallelGC` 启用Parallel垃圾收集器.
Parallel收集器关注吞吐量.

* 新生代 `Parallel Scavenge` 使用复制算法, 暂停所有用户线程.
* 老年代 `Parallel Old` 使用标记-整理算法, 暂停所有线程.
...
.CMS回收流程
. 初始标记: 标记一下 `GC Roots` 对象. `单线程` `STW`
. 并发标记: 从 `GC Roots` 对象开始遍历整个对象图.
. 重新标记: 修正并发标记期间, 因用户程序继续运作而导致标记变动的那部分对象的标记记录. `STW`
. 并发清除: 清除那些被标记为死亡的对象.

CMS回收并发线程数默认是 stem:[("nCPU"+3)/4] .

.CMS退化成Serial Old收集器回收老年代的两个诱因
* 由于CMS使用标记-清除算法, 会产生较多的内存碎片导致分配大对象的时候找不到连续的可用空间.
* CMS并发标记阶段应用线程会产生一部分新的可回收对象( _浮动垃圾_ ), 且分配新的对象内存的时候可能因为内存不足发生 `concurrent mode failure` .
...
.G1回收流程
. 初始标记: 标记 `GC Roots` 对象. `单线程` `STW`
. 并发标记: 从 `GC Roots` 对象开始遍历整个对象图.
. 最终标记: 修正并发标记期间, 因用户程序继续运作而导致标记变动的那部分对象的标记记录. `STW`
. 筛选回收: 将回收的Region存活下来的对象复制到空的Region中再清空原来的Region. `STW`
