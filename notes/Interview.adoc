...
哪些情况会导致Full GC?::
* 老年代满
* CMS回收失败, 发生 `Concurrent Mode Failure` .
* 从新生代要放入老年代的对象超过了老年代剩余空间.
频繁GC问题如何定位?::
* core dump分析
* gc日志上传到 https://gceasy.io 分析, 根据报告调整各个区内存大小.
OOM类型?::
* 堆内存溢出. `Java heap space`
* 花费在GC上的时间超过了应用总运行时间的98%. `GC overhead limit exceeded`
* 使用反射创建大量的动态代理类, 导致Metaspace溢出. `Metaspace`
* 创建线程时没有足够内存分配. `unable to create new native thread`
* 使用Unsafe分配内存时不够. `Direct buffer memory`
内存溢出和内存泄漏的区别?::
* 内存泄漏: 内存一直占用但不释放.
